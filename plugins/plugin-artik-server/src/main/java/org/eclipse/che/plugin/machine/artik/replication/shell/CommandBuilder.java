...
class CommandBuilder {
    ...
    static CommandLine buildCommandWithContext(MkdirCommandContext ctx) {
        return new CommandLine().add(SSHPASS.NAME)
                                .add(SSHPASS.PASSWORD_KEY, ctx.getPassword())

                                .add(SSH.NAME)
                                .add(SSH.PORT_KEY, ctx.getPort())

                                .add(SSH.OPTIONS_KEY)
                                .addPair(Options.STRICT_HOST_KEY_CHECKING, Options.NO)

                                .add(format("%s@%s", ctx.getUsername(), ctx.getHost()))
                                .add(MKDIR.NAME)
                                .add(MKDIR.PARENTS_KEY)
                                .add(ctx.getPath());
    }
     ...
     *
     * @see <a href="http://man7.org/linux/man-pages/man1/mkdir.1.html">MKDIR</a>
     */
    private interface MKDIR {
        String NAME        = "mkdir";
        /**
         * No error if existing, make parent directories as needed
         */
        String PARENTS_KEY = "-p";
    }

...
class ReplicationManager {
    ...
    /**
     * Creates a directory on a remote (artik) machine
     *
     * @param relativePath relative path of a folder on a remote machine,
     *                     it is added to a value of synchronization folder
     *                     parameter obtained by parsing machine's script
     * @param machineId identifier of a machine the MKDIR call is addressed to,
     *                  to get all needed information we perform a request to
     *                  {@link MachineService}
     * @throws ServerException something gone wrong when we executed a shell command
     * @throws IOException something gone wrong when we executed a shell command
     */
    void makeDirectory(String machineId, String relativePath) throws IOException, ServerException {
        shellCommandManager.mkdir(jsonValueHelperFactory.create(machineId), relativePath);
    }
}

...
public class ShellCommandManagerTest {
    ...
    private static final String MKDIR_COMMAND_LINE = format("sshpass -p %s ssh -p %s -o StrictHostKeyChecking=no %s@%s mkdir -p %s%s",
                                                            PASSWORD, PORT, USERNAME, HOST, REPLICATION, TARGET);
    private static final String SCP_COMMAND_LINE   = format("sshpass -p %s scp -P %s -o StrictHostKeyChecking=no -r %s %s@%s:%s%s",
    ...
    @Test
    public void shouldCallSMkdirCommandOnExecutor() throws IOException, ServerException {
        shellCommandManager.mkdir(jsonValueHelper, TARGET);

        final ArgumentCaptor<CommandLine> captor = forClass(CommandLine.class);

        verify(shellCommandExecutor, times(1)).execute(captor.capture());

        assertEquals(captor.getValue().toString(), MKDIR_COMMAND_LINE);
    }

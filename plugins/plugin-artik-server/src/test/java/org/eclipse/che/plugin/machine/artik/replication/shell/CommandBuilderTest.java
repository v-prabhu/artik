...
public class CommandBuilderTest {
    ...
    private static final String MKDIR_COMMAND_LINE = format("sshpass -p %s ssh -p %s -o StrictHostKeyChecking=no %s@%s mkdir -p %s",
                                                            PASSWORD, PORT, USERNAME, HOST, PATH);
    ...
    @Mock
    private MkdirCommandContext mkdirCommandContext;
    ...
    @Test
    public void shouldBuildProperMkdirCommand() {
        when(mkdirCommandContext.getUsername()).thenReturn(USERNAME);
        when(mkdirCommandContext.getPassword()).thenReturn(PASSWORD);
        when(mkdirCommandContext.getHost()).thenReturn(HOST);
        when(mkdirCommandContext.getPort()).thenReturn(PORT);
        when(mkdirCommandContext.getPath()).thenReturn(PATH);

        assertEquals(buildCommandWithContext(mkdirCommandContext).toString(), MKDIR_COMMAND_LINE);
    }
}

...
public class ReplicationManagerTest {
    ...
    @Test
    public void shouldCallShellCommandManagerMkdirOnMakeDirectory() throws ServerException, IOException, JsonParseException {
        when(jsonValueHelperFactory.create(any(String.class))).thenReturn(mock(JsonValueHelper.class));

        replicationManager.makeDirectory(MACHINE_ID, PATH_TO_DIRECTORY);

        verify(shellCommandManager, times(1)).mkdir(any(JsonValueHelper.class), eq(PATH_TO_DIRECTORY));
    }

 ...
 */
@Singleton
public class CCompileCommandProducer extends AbstractArtikProducer {

    public static final String COMMAND_NAME     = "Compile";
    public static final String COMMAND_TEMPLATE =
            "cd ${explorer.current.file.parent.path} && ${c.compilation.options} -o ${binary.name} ${explorer.current.file.name} && echo 'Compilation completed'";

    @Inject
    public CCompileCommandProducer(CustomCommandType customCommandType, DtoFactory dtoFactory, AppContext appContext) {
        super(COMMAND_NAME, Constants.C_PROJECT_TYPE_ID, customCommandType, dtoFactory, appContext);
    }
    ...
    public boolean isApplicable() {
        ...
        if (selectedResource.isFile()) {
            ...
            return Constants.C_EXT.equals(ext);
        }
